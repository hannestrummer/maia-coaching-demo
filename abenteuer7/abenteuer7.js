...
function videoCard(label, vid, caption) { const p = (window.MAIA_POSTER || {})[vid]; add(`<div class="card vcard"><video controls preload="metadata" playsinline${p ? ` poster="${p}"` : ""} src="${VBASE}${vid}.mp4"></video><div class="vmeta"><b>${label}</b>${caption ? `<span>${caption}</span>` : ""}</div></div>`); scroll(); }
